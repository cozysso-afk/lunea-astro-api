...
def _motion_arrays(body, sample_times):
    ...
    now = get_tropical_ecliptic_lons(body, sample_times)
    past = get_tropical_ecliptic_lons(body, past_times)
    future = get_tropical_ecliptic_lons(body, future_times)
    speed = circular_delta_array(future, past) / ((2*h) / 24.0)
...
def _score_sample(topic, spec, targets, asc_lon, placidus_cusps, body_rows, idx):
    ...
    for body, rows in body_rows.items():
        lon_now = float(rows["now"][idx])
        lon_past = float(rows["past"][idx])
        lon_future = float(rows["future"][idx])
        speed = float(rows["speed"][idx])
